...
1. `pg_publication_sync` 对象级记录 (`pfsynckind='o'`) 的 `message_type` 使用单字符协议：
   * `Q`: Query SQL 重放（已实现）
   * `A`: publication 成员新增（已实现）
   * `D`: publication 成员移除（已实现）
...
3. 订阅端 apply worker 在接收 `pg_publication_sync` 行后：
   * 先按既有逻辑写入系统表；
   * 再按 `message_type` 进入分发器（前置过滤：`subddl` 命中 + `publication_list` 与订阅 publication 列表有交集）；
   * `Q` 分支执行 `ddl_str`；
   * `A` 分支把 `target_table` 加入本地 `pg_subscription_rel`；
   * `D` 分支把 `target_table` 从本地 `pg_subscription_rel` 移除。

4. 分发器只对 `pg_publication_sync` 且 `pfsynckind='o'` 生效，不影响 publication 级元数据行和普通表复制链路。
5. 捕获端过滤规则：仅当 publication 同时满足
   * `with (ddl=...)` 类型命中；
   * 对象范围命中（`FOR TABLE` / `FOR TABLES IN SCHEMA` / `FOR ALL TABLES`）；
   才会把 publication 名写入 `publication_list`。
